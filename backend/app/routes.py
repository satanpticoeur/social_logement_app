...
from app import db
from app.models import Utilisateur, Maison, Chambre, Contrat, Paiement, RendezVous, Media, Probleme
...
def serialize_maison(maison):
    # Inclut le propriétaire pour la user story 10
    proprietaire_data = None
    if maison.proprietaire:  # Vérifie si la relation est chargée
        proprietaire_data = serialize_utilisateur(maison.proprietaire)

    return {
        'id': maison.id,
        'proprietaire_id': maison.proprietaire_id,
        'proprietaire': proprietaire_data,  # Ajout des infos du propriétaire
        'adresse': maison.adresse,
        'latitude': str(maison.latitude) if maison.latitude else None,
        'longitude': str(maison.longitude) if maison.longitude else None,
...
def serialize_chambre(chambre):
    # Inclut la maison et le propriétaire pour les user stories 7 et 10
    maison_data = None
    if chambre.maison:
        maison_data = serialize_maison(chambre.maison)  # Utilise la fonction serialize_maison pour la maison

    # Inclure les médias de la chambre
    medias_data = [serialize_media(m) for m in chambre.medias] if chambre.medias else []

    return {
        'id': chambre.id,
        'maison_id': chambre.maison_id,
        'maison': maison_data,  # Ajout des infos de la maison
        'titre': chambre.titre,
...
def serialize_rendezvous(rendezvous):
    return {
        'id': rendezvous.id,
        'locataire_id': rendezvous.locataire_id,
        'chambre_id': rendezvous.chambre_id,
        'date_heure': rendezvous.date_heure.isoformat() if rendezvous.date_heure else None,
        'statut': rendezvous.statut,
        'cree_le': rendezvous.cree_le.isoformat() if rendezvous.cree_le else None
    }
